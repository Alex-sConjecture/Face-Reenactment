...
class SAEModel(ModelBase):
    ...
    def onInitialize(self):
        ...
        if self.options['archi'] == 'liae':
            self.encoder = modelify(SAEModel.LIAEEncFlow(resolution, self.options['lighter_encoder'], ed_ch_dims=ed_ch_dims, **common_flow_kwargs)  ) (Input(bgr_shape))
            ...
            self.decoder = modelify(SAEModel.LIAEDecFlow (bgr_shape[2],ed_ch_dims=ed_ch_dims//2, multiscale_count=self.ms_count, **common_flow_kwargs)) (inter_output_Inputs)

            models_list += [self.encoder, self.inter_B, self.inter_AB, self.decoder]

            if self.options['learn_mask']:
                self.decoderm = modelify(SAEModel.LIAEDecFlow (mask_shape[2],ed_ch_dims=int(ed_ch_dims/1.5), **common_flow_kwargs)) (inter_output_Inputs)
            ...
            self.encoder = modelify(SAEModel.DFEncFlow(resolution, self.options['lighter_encoder'], ae_dims=ae_dims, ed_ch_dims=ed_ch_dims, **common_flow_kwargs)  ) (Input(bgr_shape))

            dec_Inputs = [ Input(K.int_shape(x)[1:]) for x in self.encoder.outputs ]

            self.decoder_src = modelify(SAEModel.DFDecFlow (bgr_shape[2],ed_ch_dims=ed_ch_dims//2, multiscale_count=self.ms_count, **common_flow_kwargs )) (dec_Inputs)
            self.decoder_dst = modelify(SAEModel.DFDecFlow (bgr_shape[2],ed_ch_dims=ed_ch_dims//2, multiscale_count=self.ms_count, **common_flow_kwargs )) (dec_Inputs)

            models_list += [self.encoder, self.decoder_src, self.decoder_dst]

            if self.options['learn_mask']:
                self.decoder_srcm = modelify(SAEModel.DFDecFlow (mask_shape[2],ed_ch_dims=int(ed_ch_dims/1.5), **common_flow_kwargs )) (dec_Inputs)
                self.decoder_dstm = modelify(SAEModel.DFDecFlow (mask_shape[2],ed_ch_dims=int(ed_ch_dims/1.5), **common_flow_kwargs )) (dec_Inputs)
            ...
            self.encoder = modelify(SAEModel.VGEncFlow(resolution, self.options['lighter_encoder'], ae_dims=ae_dims, ed_ch_dims=ed_ch_dims, **common_flow_kwargs)  ) (Input(bgr_shape))

            dec_Inputs = [ Input(K.int_shape(x)[1:]) for x in self.encoder.outputs ]

            self.decoder_src = modelify(SAEModel.VGDecFlow (bgr_shape[2],ed_ch_dims=ed_ch_dims//2, **common_flow_kwargs )) (dec_Inputs)
            self.decoder_dst = modelify(SAEModel.VGDecFlow (bgr_shape[2],ed_ch_dims=ed_ch_dims//2, **common_flow_kwargs )) (dec_Inputs)

            models_list += [self.encoder, self.decoder_src, self.decoder_dst]

            if self.options['learn_mask']:
                self.decoder_srcm = modelify(SAEModel.VGDecFlow (mask_shape[2],ed_ch_dims=int(ed_ch_dims/1.5), **common_flow_kwargs )) (dec_Inputs)
                self.decoder_dstm = modelify(SAEModel.VGDecFlow (mask_shape[2],ed_ch_dims=int(ed_ch_dims/1.5), **common_flow_kwargs )) (dec_Inputs)
                models_list += [self.decoder_srcm, self.decoder_dstm]
    ...
    @staticmethod
    def LIAEEncFlow(resolution, light_enc, ed_ch_dims=42, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        ...
        def func(input):
            ...
            if not light_enc:
                x = downscale(ed_dims*2, padding=padding)(x)
                x = downscale(ed_dims*4, padding=padding)(x)
                x = downscale(ed_dims*8, padding=padding)(x)
            else:
                x = downscale_sep(ed_dims*2, padding=padding)(x)
                x = downscale(ed_dims*4, padding=padding)(x)
                x = downscale_sep(ed_dims*8, padding=padding)(x)
    ...
    def LIAEInterFlow(resolution, ae_dims=256, padding='zero', **kwargs):
        ...
        def func(input):
            ...
            x = upscale(ae_dims*2, padding=padding)(x)
            return x
        return func

    @staticmethod
    def LIAEDecFlow(output_nc,ed_ch_dims=21, multiscale_count=1, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        upscale = SAEModel.upscale
        to_bgr = SAEModel.to_bgr
        ed_dims = output_nc * ed_ch_dims
        ResBlock2 = SAEModel.ResBlock2

        def func(input):
            x = input[0]

            outputs = []
            x  = upscale(ed_dims*8, padding=padding)( x )
            x1 = ResBlock2( ed_dims*8, padding=padding )(x)

            if multiscale_count >= 3:
                outputs += [ to_bgr(output_nc) ( x1 ) ]

            x1 = upscale(ed_dims*4, padding=padding)( x1 )
            x2 = ResBlock2( ed_dims*4 , padding=padding)(x1)

            if multiscale_count >= 2:
                outputs += [ to_bgr(output_nc) ( x2 ) ]

            x2 = upscale(ed_dims*2, padding=padding)( x2 )
            x3 = ResBlock2( ed_dims*2, padding=padding )(x2)
    ...
    @staticmethod
    def DFEncFlow(resolution, light_enc, ae_dims=512, ed_ch_dims=42, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        upscale = SAEModel.upscale
        downscale = SAEModel.downscale
        downscale_sep = SAEModel.downscale_sep
        ...
        def func(input):
            ...
            if not light_enc:
                ...
                x = downscale_sep(ed_dims*2, padding=padding)(x)
                x = downscale_sep(ed_dims*4, padding=padding)(x)
                x = downscale_sep(ed_dims*8, padding=padding)(x)

            x = Dense(ae_dims)(Flatten()(x))
            x = Dense(lowest_dense_res * lowest_dense_res * ae_dims)(x)
            x = Reshape((lowest_dense_res, lowest_dense_res, ae_dims))(x)
            x = upscale(ae_dims, padding=padding)(x)

            return x
        return func

    @staticmethod
    def DFDecFlow(output_nc, ed_ch_dims=21, multiscale_count=1, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        upscale = SAEModel.upscale
        to_bgr = SAEModel.to_bgr
        ed_dims = output_nc * ed_ch_dims
    ...
    @staticmethod
    def VGEncFlow(resolution, light_enc, ae_dims=512, ed_ch_dims=42, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        upscale = SAEModel.upscale
        downscale = SAEModel.downscale
        downscale_sep = SAEModel.downscale_sep
        ...
        def func(input):
            ...
            x = tmp_x = Conv2D (in_conv_filters, kernel_size=5, strides=2, padding='reflect') (x)

            for _ in range ( 8 if light_enc else 16 ):
                x = ResidualBlock(ed_dims, padding=padding)(x)

            x = Add()([x, tmp_x])

            x = downscale(ed_dims, padding=padding)(x)
    ...
    @staticmethod
    def VGDecFlow(output_nc, ed_ch_dims=21, multiscale_count=1, padding='zero', **kwargs):
        exec (nnlib.import_all(), locals(), globals())
        upscale = SAEModel.upscale
        to_bgr = SAEModel.to_bgr
        ResidualBlock = SAEModel.ResidualBlock
        ...
        def func(input):
            ...
            x = upscale( ed_dims*8, padding=padding )(x)
            x = ResidualBlock( ed_dims*8, padding=padding )(x)

            x = upscale( ed_dims*4, padding=padding )(x)
            x = ResidualBlock( ed_dims*4, padding=padding )(x)

            x = upscale( ed_dims*2, padding=padding )(x)
            x = ResidualBlock( ed_dims*2, padding=padding )(x)
